...
import {
  Package, MessageSquare, ShoppingBag, TrendingUp, Plus, ArrowRight,
  Clock, CheckCircle, Truck, XCircle, X, MapPin, CreditCard, Phone, Flame, Trash2,
} from 'lucide-react';
...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      const [
        ...
        supabase.from('products').select('*').eq('is_daily_deal', true).eq('is_active', true).order('updated_at', { ascending: false }),
  ...
  const viewOrderDetails = async (orderNumber: string) => {
    ...
    const { data } = await supabase
      ...
      .maybeSingle();
    if (data) setSelectedOrder(data as OrderDetails);
    setOrderModalLoading(false);
  };

  const toggleDailyDeal = async (product: Product) => {
    const newDealStatus = !product.is_daily_deal;
    const { error } = await supabase
      .from('products')
      .update({ is_daily_deal: newDealStatus })
      .eq('id', product.id);

    if (error) {
      toast.error('Failed to update deal status');
    } else {
      setAllProducts((prev) => prev.map((p) => p.id === product.id ? { ...p, is_daily_deal: newDealStatus } : p));
      if (newDealStatus) {
        setDailyDealsProducts((prev) => [...prev, { ...product, is_daily_deal: true }]);
      } else {
          ...
          <div className="space-y-2 max-h-80 overflow-y-auto">
            {allProducts.map((p) => (
              <div key={p.id} className="flex items-center gap-3 p-2 rounded-lg bg-white/3 border border-white/5">
                ...
                >
                  {p.is_daily_deal ? 'In Deals' : 'Add to Deals'}
                </button>
              </div>
            ))}
          </div>
        ...
        {recentOrders.length === 0 ? (
          ...
          <div className="space-y-2">
            {recentOrders.map((order) => {
              ...
              return (
                <div key={order.order_number} className="flex items-center justify-between p-3 bg-white/3 rounded-xl border border-white/5">
                  <div>
            ...
            >
              ...
              <div className="flex items-center justify-between p-5 border-b border-white/5">
                <div>
                  ...
                  <div className="text-sm text-gold-400">{selectedOrder.order_number}</div>
                </div>
              ...
              <div className="p-5 space-y-5">
                ...
                <div className="flex items-center justify-between">
                  <div className="text-sm text-silver-500">
                    {new Date(selectedOrder.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                  </div>
                ...
                <div>
                  ...
                  <div className="bg-white/3 rounded-lg p-3 space-y-1">
                    <div className="text-sm font-medium text-white">{selectedOrder.shipping_full_name}</div>
                    <div className="text-xs text-silver-400">{selectedOrder.shipping_line1}</div>
                    {selectedOrder.shipping_line2 && <div className="text-xs text-silver-400">{selectedOrder.shipping_line2}</div>}
                    <div className="text-xs text-silver-400">{selectedOrder.shipping_city}, {selectedOrder.shipping_state} - {selectedOrder.shipping_pincode}</div>
                    <div className="text-xs text-silver-400 flex items-center gap-1 mt-1">
                      <Phone size={11} /> {selectedOrder.shipping_phone}
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="bg-white/3 rounded-lg p-3">
                    <div className="text-sm text-white capitalize">{selectedOrder.payment_method}</div>
                    <div className="text-xs text-silver-500">Status: {selectedOrder.payment_status}</div>
                  </div>
                </div>
